...
class user : public AnalyzerBase
{
 ...
 private : 
  ...
  bool isP__zPTorderinginterstate(const MCParticleFormat* part) const {
  ...
  bool isP__zPTorderingallstate(const MCParticleFormat* part) const {
     if ( part==0 ) return false;
     if ( (part->pdgid()!=23) ) return false;
     return true; }
  std::vector<const MCParticleFormat*> _P_aPTorderingallstate_REG_;
  bool isP__aPTorderingallstate(const MCParticleFormat* part) const {
     if ( part==0 ) return false;

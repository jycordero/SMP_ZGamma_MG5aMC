...
C      2*Epsilon(1,3,-1,-2)*P(-2,2)*P(-1,1)*P(2,3) -
C      Epsilon(1,2,-1,-2)*P(-2,3)*P(-1,2)*P(3,1) - 2*Epsilon(1,2,-1,-2)
C     *P(-2,3)*P(-1,1)*P(3,2) + Epsilon(3,-1,-2,-3)*P(-3,3)*P(-2,2)*P(-
C     1,1)*Metric(1,2) - Epsilon(2,-1,-2,-3)*P(-3,3)*P(-2,2)*P(-1,1)*Me
C     tric(1,3)
C     
     ...
     $ *TMP19)+CI*(TMP6*TMP7+TMP12*TMP13+TMP16*TMP17+TMP20*TMP21)+2D0 
     $ * CI*(TMP0*TMP1+TMP2*TMP3))
      END
